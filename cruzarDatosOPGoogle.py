    ...
    for i,x in enumerate(openTable['Google Fecha']):
        if openTable['Fuente'][i] == 'Tu red':
            ...
            for j,y in enumerate(google['Fecha y hora (AAAAMMDDHH)']):
                if x == y:  
                    if contiene(google['Primera fuente/medio del usuario'][j]) == True:
                        print(1)
                        if google['Tiempo de interacción medio'][j] >= mayorTiempo:
                            mayorTiempo = google['Tiempo de interacción medio'][j]
                            pauta = google['Primera fuente/medio del usuario'][j]
...
def tablaInfo(csv_path):
    ...
    df_original.to_csv('Oh Mex OD con Pauta.csv', index=False)
...
def main():
    ...
    google = pd.read_csv('Google Exports/Oh Mexico Junio.csv', header=9)
    openTable = pd.read_csv('Open Table Exports/Oh Mex OD.csv')
